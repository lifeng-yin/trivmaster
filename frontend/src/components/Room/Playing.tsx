...
const Countdown = ({ round }: { round: Round }) => {
  ...
  const [durationUntilFinished, setDurationUntilFinished] = useState(
    new Date(+Date.parse(round.dateFinished) - Date.now())
  );

  useEffect(() => {
    const interval = setInterval(() => {
      setDurationUntilFinished(
        new Date(+Date.parse(round.dateFinished!) - Date.now())
      );
    }, 1000);
  ...
  return (
    <div>
      <p>
        {durationUntilFinished.getMinutes()}:
        {padZeroes(durationUntilFinished.getSeconds())}
      </p>
    </div>
  );
};
...
const Playing = ({
  ...
  currentQuestion,
  currentAnswers,
  currentCorrectUsername,
}: {
  round: Round;
  isAnswering: boolean;
  currentQuestion: string;
  currentAnswers: string[];
  currentCorrectUsername: string;
}) => {
  const [typedAnswer, setTypedAnswer] = useState("");

  const handleSubmit = (e) => {
    e.preventDefault();
    socket.emit("round:answer", typedAnswer);
  };
