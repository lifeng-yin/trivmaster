...
function Room() {
  ...
  useEffect(() => {
    ...
    socket.on(
      ...
      (
        ...
        newTeam1: String[],
        newTeam2: String[],
        newRoundSettings: RoundSettings
      ) => {
        setUser(newUser);
    ...
    socket.on("update-questions", (newQuestions: Question[]) =>
      setQuestions(newQuestions)
    );
    ...
    socket.on("update-round-settings", (newRoundSettings: RoundSettings) =>
      setRoundSettings(newRoundSettings)
    );
    ...
    socket.on("error:room-deleted", () => {
      navigate("/join", {
        state: { error: "Error: Room unavailable as the owner has left." },
      });
    });
  ...
  function Sidebar() {
    return (
      <nav className="absolute top-32 bg-white z-20">
        <div>
  ...
  const activePageComponents = {
    ...
    playing: (
      <Playing
        ...
        currentCorrectUsername={currentCorrectUsername}
      />
    ),
  };
